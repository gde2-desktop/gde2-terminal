 ...
 *   /apps/gde2-terminal/global/
 *   /apps/gde2-terminal/profiles/Foo/
 *
 ...
 * these categories. See gde2-terminal 1.x for why.
 *
...
#define MONOSPACE_FONT_SCHEMA "org.gde2.interface"
                             ...
                             TerminalProfile *profile)
{
	...
	gde2_gsettings_remove_all_from_strv (app->settings_global, PROFILE_LIST_KEY, profile_name);

	/* And remove the profile directory */
	if (!gde2_dconf_recursive_reset (profile_dir, &error))
	{
                         ...
                         TerminalApp *app)
{
	if (response_id == GTK_RESPONSE_ACCEPT)
	{
		...
		gde2_gsettings_append_strv (app->settings_global,
						PROFILE_LIST_KEY,
						new_profile_name);
                          ...
                          TerminalApp *app)
{
	...
	{
		terminal_util_show_help ("gde2-terminal-manage-profiles", GTK_WINDOW (dialog));
		return;
	}
...
terminal_app_init (TerminalApp *app)
{
	...
	gtk_window_set_default_icon_name (GDE2_TERMINAL_ICON_NAME);
